            ...
            if (_pandas == null)
            {
                using (Py.GIL())
                {
                    ...
                    _pandas = PythonEngine.ModuleFromString("remapper",
                        @"import pandas as pd
from pandas.core.groupby.generic import DataFrameGroupBy as pdDataFrameGroupBy
...
def replace_groupby(cls, fcls):
    '''Replaces the groupby of a given class by g that wraps fcls
    This is how pandas implements groupby. In this case, we wrap
    SeriesGroupby or DataFrameGroupBy classes before.
    '''
    fcls = CreateWrapperClass(fcls)
    def g(*args, **kwargs):
        return fcls(*args, **kwargs)
    g.__name__ = 'groupby'
    setattr(cls, 'groupby', g)
...
replace_groupby(Series, pdSeriesGroupBy)
replace_groupby(DataFrame, pdDataFrameGroupBy)
